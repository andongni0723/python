...
betMoney = 0
betPlayer = 1
PlayerList = [[Player(1, "A"), 0], [Player(2, "B"), 0], [Player(3, "C"), 0], [Player(4, "D"), 0], [Player(5, "E"), 0],
              [Player(6, "F"), 0]]  # [player data, run distance]
...
def INIT():
    ...
    betMoney = 0
    betPlayer = 1
    PlayerList = [[Player(1, "A"), 0], [Player(2, "B"), 0], [Player(3, "C"), 0], [Player(4, "D"), 0],
                  [Player(5, "E"), 0], [Player(6, "F"), 0]]  # [player data, run distance]
...
def Race():
    ...
    while not (isDone):
        ...
        if RankList[0][1] >= 100:
            ...
            rd3 = f"{RankList[2][0].index}. {RankList[2][0].name}" if (
                    startPlayer > 2) else ""  # if only 2 players, the race hasn't 3rd
...
def PrintRank(_RankList):
    ...
    for i in range(startPlayer):
        print(
            f"{i + 1}st.\t| {_RankList[i][0].index}\t| {_RankList[i][0].name}\t| {_RankList[i][0].speed} m/s \t| {_RankList[i][1]} m\n"
            "--------------------------------------------------")
